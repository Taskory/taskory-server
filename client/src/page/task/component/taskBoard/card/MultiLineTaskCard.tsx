    ...
    return (
        ...
        >
            <div className="flex justify-between items-center mb-1">
                <p className="text-gray-900 font-medium text-sm truncate" title={task.title}>
                    {task.title}
                </p>
                <div className="flex items-center gap-1">
                    <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium truncate ${getTagColorStyle(task.tagColor ?? "")}`}
                          title={task.tagTitle ?? ""}>
            ...
            <div className="flex justify-between items-center mb-1">
                <p className="text-xs text-gray-500 truncate" title={task.event?.title}>
                    <span className="font-semibold">Event:</span> {task.event?.title ?? "No Event"}
                </p>
            </div>
