...
import { createEvent, updateEvent, getEventById } from "../../api/event/EventApi";
...
export const CalendarModal = forwardRef<HTMLDialogElement, CalendarModalProps>(({ eventId }, ref) => {
    ...
    const handleSave = async () => {
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                console.log(isEditMode ? "Event successfully updated:" : "Event successfully created:", response.data);
                handleClose();
            } else {
    ...
    return (
        <dialog ref={ref} className="modal">
            <div className="modal-box w-11/12 max-w-5xl">
                <h3 className="font-bold text-lg">{isEditMode ? "Edit Event" : "Create Event"}</h3>
